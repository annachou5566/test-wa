#!/usr/bin/env python3
"""One-shot public-safe ETH issuer artifact probe; no Wave Alpha private source/secrets."""
...
OUT = Path('artifacts/eth-tier1-live')
...
SOURCES = {
    'ETHA': {
        ...
        'method': 'requests', 'suffix': '.csv',
        'markers': ['Fund Holdings as of', 'Shares Outstanding', 'Ticker,Name', 'ETH', 'ETHER'],
    },
    'ETHB': {
        'url': 'https://www.ishares.com/us/products/348532/ishares-staked-ethereum-trust-etf/latest-holdings.csv',
        'method': 'requests', 'suffix': '.csv',
        'markers': ['Fund Holdings as of', 'Shares Outstanding', 'Ticker,Name', 'STAKED ETHER', 'ETHER'],
    },
    'ETHW': {
        'url': 'https://ethwetf.com/',
        'method': 'requests', 'suffix': '.html',
        'markers': ['Shares Outstanding', 'Fund Holdings', 'ETH in Trust', 'ETH per Share'],
    },
    'ETHV': {
        'url': 'https://www.vaneck.com/us/en/investments/ethereum-etf-ethv/',
        'method': 'browser_text', 'suffix': '.txt',
        'markers': ['ETF Statistics', 'Shares Outstanding', 'Ether in Trust', 'Ether per Basket'],
    },
    'EZET': {
        'url': 'https://www.franklintempleton.com/investments/options/exchange-traded-funds/products/40521/SINGLCLASS/franklin-ethereum-etf/EZET',
        'method': 'browser_text', 'suffix': '.txt',
        'markers': ['Shares Outstanding', 'Ether in Fund', 'Ether per Basket'],
    },
}
...
def click_if_visible(page, patterns: list[str]) -> list[str]:
    clicked = []
...
def browser_capture(browser, ticker: str, spec: dict) -> dict:
    ...
    try:
        ...
        try:
            ...
            pass
        clicks = click_if_visible(page, [r'accept all|accept|agree', r'individual investor|retail investor|continue as.*investor'])
        for _ in range(8):
...
def main() -> None:
    summary = {
        'probe': 'US_ETH_TIER1_LIVE_ACQUISITION',
        'started_at_utc': now(),
